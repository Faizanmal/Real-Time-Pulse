...
export class RecaptchaService {
  ...
  async verifyToken(
    ...
    const enabled = this.configService.get<boolean>('security.recaptcha.enabled');

    if (!enabled) {
      return { valid: true };
    }

    const secretKey = this.configService.get<string>('security.recaptcha.secretKey');
    ...
    try {
      ...
      const response = await firstValueFrom(
        this.httpService.post<RecaptchaResponse>(this.VERIFY_URL, params.toString(), {
          headers: {
      ...
      const minScore = this.configService.get<number>('security.recaptcha.minScore') || 0.5;
   ...
   */
  async validateOrThrow(token: string, action?: string, remoteIp?: string): Promise<void> {
