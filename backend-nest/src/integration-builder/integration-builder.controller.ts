...
export class IntegrationBuilderController {
  ...
  async createIntegration(
    @Body() body: { workspaceId: string; integration: any },
  ) {
  ...
  async importFromOpenAPI(
    @Body() body: { workspaceId: string; openAPISpec: any; name: string },
  ) {
  ...
  async setupOAuth2(
    ...
    @Body() oauth2Config: any,
  ) {
    const authUrl = await this.integrationBuilderService.setupOAuth2(
      integrationId,
      oauth2Config,
    );
    return { authUrl };
  ...
  async handleOAuth2Callback(
    ...
    return await this.integrationBuilderService.handleOAuth2Callback(
      integrationId,
      body.code,
    );
  }
  ...
  async addTransformation(
    @Param('id') integrationId: string,
    @Body() transformation: any,
  ) {
    return await this.integrationBuilderService.addTransformation(
      integrationId,
      transformation,
    );
  }
  ...
  async createCustomWidget(
    ...
    return await this.integrationBuilderService.createCustomWidget(
      integrationId,
      widget,
    );
  }
  ...
  async testEndpoint(
    ...
    return await this.integrationBuilderService.testEndpoint(
      integrationId,
      endpointId,
      testData,
    );
