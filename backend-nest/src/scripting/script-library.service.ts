...
export class ScriptLibraryService {
    ...
    return {
      ...
      diff: (date1: Date, date2: Date, unit: string = 'days') => {
        ...
        switch (unit) {
          ...
          case 'months': return Math.floor(diffMs / 2592000000);
          case 'years': return Math.floor(diffMs / 31536000000);
  ...
  private getStringLibrary(): Record<string, Function> {
    return {
      ...
      kebabCase: (str: string) =>
        String(str).replace(/([a-z])([A-Z])/g, '$1-$2')
          .replace(/[\s_]+/g, '-').toLowerCase(),
      snakeCase: (str: string) =>
        String(str).replace(/([a-z])([A-Z])/g, '$1_$2')
  ...
  private getFormatLibrary(): Record<string, Function> {
    return {
      ...
      number: (n: number, decimals: number = 0) =>
        n.toFixed(decimals),
      currency: (n: number, currency: string = 'USD', locale: string = 'en-US') =>
  ...
  private getTransformLibrary(): Record<string, Function> {
    return {
      ...
      unpivot: (obj: Record<string, any>, keyName: string = 'key', valueName: string = 'value') =>
        Object.entries(obj).map(([key, value]) => ({ [keyName]: key, [valueName]: value })),
      
      // Aggregation
      aggregate: (arr: any[], groupBy: string, aggregations: Record<string, { field: string; fn: 'sum' | 'avg' | 'min' | 'max' | 'count' }>) => {
        ...
        return Object.entries(groups).map(([key, items]) => {
          ...
          for (const [name, agg] of Object.entries(aggregations)) {
            const values = items.map(i => i[agg.field]).filter(v => v !== undefined);
            switch (agg.fn) {
              case 'sum':
                result[name] = values.reduce((a, b) => a + b, 0);
                break;
              case 'avg':
                result[name] = values.length ? values.reduce((a, b) => a + b, 0) / values.length : 0;
                break;
              case 'min':
